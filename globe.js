/* ===========================================================================
   ...
   Pure vanilla canvas — no dependencies.
   =========================================================================== */
(function () {
  "use strict";
  ...
  var W, H, DPR, cx, cy, R;
  var TILT = -0.42;                 // tip the north pole toward the viewer
  ...
  // network of links between cities (by index)
  var edges = [
  ...
  // project a (possibly non-unit) vector: rotate around Y, tilt around X,
  // orthographic projection to screen. Returns screen x/y and depth z.
  ...
  // cache arc sample vectors (great circle + altitude lift) once
  var ARC_STEPS = 46;
  ...
  function drawGrid(cosR, sinR) {
    ctx.lineWidth = 1;
    ctx.strokeStyle = "rgba(150,125,255,0.16)";
    var lat, lon, pts;
    // parallels
  ...
  function drawArc(arc, cosR, sinR, time) {
    ...
    // glowing line (front-facing portion only)
    ctx.save();
    ctx.shadowColor = "rgba(124,90,255,0.9)";
    ctx.shadowBlur = 8;
    ctx.lineWidth = 1.2;
    ...
    if (pp && pp.z > 0) {
      ctx.save();
      ctx.shadowColor = "rgba(170,215,255,1)";
      ctx.shadowBlur = 12;
  ...
  function draw(rot, time) {
    ...
    // atmosphere halo
    var halo = ctx.createRadialGradient(cx, cy, R * 0.65, cx, cy, R * 1.35);
    halo.addColorStop(0, "rgba(90,120,255,0)");
    halo.addColorStop(0.72, "rgba(96,96,220,0.08)");
    halo.addColorStop(0.9, "rgba(120,150,255,0.16)");
    ...
    body.addColorStop(0, "rgba(48,44,98,0.55)");
    body.addColorStop(0.7, "rgba(20,17,48,0.55)");
    body.addColorStop(1, "rgba(8,8,24,0.62)");
    ctx.fillStyle = body;
    ctx.beginPath();
